...
from scapy.fields import (ByteField, ConditionalField, StrFixedLenField, FlagsField,
                          ...
                          PacketLenField, XByteField, SignedIntField, MultipleTypeField)
...
class SAPMSAdmRecord(PacketNoPadded):
    ...
    fields_desc = [
        ...
        MultipleTypeField(
            [
                (StrFixedLenField("record", None, 75),
                 lambda pkt: pkt.opcode not in [0x01, 0x15, 0x2e] and pkt.executed == 0x01),
                (StrFixedLenField("record_pad", None, 25),
                 lambda pkt: pkt.opcode not in [0x01, 0x15, 0x2e] and pkt.executed == 0x01),
                (StrFixedLenField("record", None, 100),
                 lambda pkt: pkt.opcode not in [0x01, 0x15, 0x2e] and pkt.executed == 0x00),
            ],
            StrFixedLenField("record", None, 100)  # Default field
        ),
...
class SAPMSProperty(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(StrLenField("value", b"", length_from=lambda pkt:pkt.value_len), lambda pkt:pkt.id in [0x04]),
...
class SAPMS(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(StrLenField("ip_to_name", b"", length_from=lambda pkt:pkt.logonname_length), lambda pkt:pkt.opcode == 0x46),
