...
class SAPDiagDyntAtomItem(PacketNoPadded):
    ...
    fields_desc = [
        ...
        # DIAG_DGOTYP_FNAME or DIAG_DGOTYP_XMLPROP
        ConditionalField(StrLenField("text", "", length_from=lambda pkt:pkt.atom_length - 13), lambda pkt:pkt.etype in [114, 120]),
        # DIAG_DGOTYP_PUSHBUTTON_2 */
        ConditionalField(ByteField("v_length", 0), lambda pkt:pkt.etype in [115]),
        ConditionalField(ByteField("v_height", 0), lambda pkt:pkt.etype in [115]),
        ConditionalField(ShortField("function_code_offset", 0), lambda pkt:pkt.etype in [115]),
        ConditionalField(ShortField("text_offset", 0), lambda pkt:pkt.etype in [115]),
        ConditionalField(StrField("text", ""), lambda pkt:pkt.etype in [115]),
        ConditionalField(StrField("function_code", ""), lambda pkt:pkt.etype in [115]),
        # DIAG_DGOTYP_TABSTRIP_BUTTON
        ConditionalField(ByteField("v_length", 0), lambda pkt:pkt.etype in [116]),
        ConditionalField(ByteField("v_height", 0), lambda pkt:pkt.etype in [116]),
        ConditionalField(ByteField("page_id", 0), lambda pkt:pkt.etype in [116]),
        ConditionalField(ShortField("function_code_offset", 0), lambda pkt:pkt.etype in [116]),
        ConditionalField(ShortField("text_offset", 0), lambda pkt:pkt.etype in [116]),
        ConditionalField(ShortField("id_offset", 0), lambda pkt:pkt.etype in [116]),
        ConditionalField(StrNullField("text", ""), lambda pkt:pkt.etype in [116]),
        ...
        ConditionalField(StrLenField("text", "", length_from=lambda pkt:pkt.atom_length - 17), lambda pkt:pkt.etype in [127]),
        # DIAG_DGOTYP_RADIOBUTTON_3
        ConditionalField(ByteField("button", 0), lambda pkt:pkt.etype in [129]),
        ConditionalField(ShortField("visible_label_length", 0), lambda pkt:pkt.etype in [129]),
        ConditionalField(ShortField("event_id_off", 0), lambda pkt:pkt.etype in [129]),
        ConditionalField(ByteField("event_id_len", 0), lambda pkt:pkt.etype in [129]),
        ConditionalField(ShortField("text_off", 0), lambda pkt:pkt.etype in [129]),
        ConditionalField(ShortField("text_length", 0), lambda pkt:pkt.etype in [129]),
        ConditionalField(StrLenField("text", "", length_from=lambda pkt:pkt.text_length), lambda pkt:pkt.etype in [129]),
        # DIAG_DGOTYP_EFIELD_2 or DIAG_DGOTYP_OFIELD_2 or DIAG_DGOTYP_KEYWORD_2
        ConditionalField(ShortField("flag1", 0), lambda pkt:pkt.etype in [130, 131, 132]),
        ConditionalField(FieldLenField("dlen", None, fmt="B", length_of="text"), lambda pkt:pkt.etype in [130, 131, 132]),
        ConditionalField(ByteField("mlen", 0), lambda pkt:pkt.etype in [130, 131, 132]),
        ConditionalField(ShortField("maxnrchars", 0), lambda pkt:pkt.etype in [130, 131, 132]),
        ConditionalField(StrLenField("text", "", length_from=lambda pkt:pkt.dlen), lambda pkt:pkt.etype in [130, 131, 132]),
        ]
